...
system(paste0("mkdir ", file.path(rootdir, "datasets", "mDats", "MNAR2")))
system(paste0("mkdir ", file.path(rootdir, "datasets", "cDats", "MNAR2")))
system(paste0("mkdir ", file.path(rootdir, "datasets", "trueEff", "MNAR2")))

proportionList <- c(10, 30, 50)
system(paste0("mkdir ", file.path(rootdir, "datasets", "trueEff", "MNAR2", proportionList[1])))
system(paste0("mkdir ", file.path(rootdir, "datasets", "trueEff", "MNAR2", proportionList[2])))
system(paste0("mkdir ", file.path(rootdir, "datasets", "trueEff", "MNAR2", proportionList[3])))
iter <- 1000
effOR <- 0.5
...
for (w in 1:3) {
    ...
    TrueFit <- coxph(Surv(time, event) ~ treat + genderf + reth_black + reth_hisp + reth_oth + practypec + b.ecogvalue + smokey + dgradeh + surgery + site_ureter + site_renal + site_urethra + age + newVar, data = wdat)
    ...
    for (i in 1:iter) {
        ...
        zetaInd <- which(names(X) %in% c("zeta"))
        miss.coef <- c(rep(log(1.1), ncol(X)))
        miss.coef[etInd] <- 0
        miss.coef[ecogInd] <- log(5)
        miss.coef[zetaInd] <- log(5)
        ...
        wdatNA <- select(wdat, -event, -hazard, -time, -reth_black, -reth_hisp, -reth_oth, -site_ureter, -site_renal, -site_urethra, -treat, -b.ecogvalue, -newVar, -zeta)
        ...
        system(paste0("mkdir ", file.path(rootdir, "datasets", "mDats", "MNAR2", propName)))
        ...
        write.csv(trueEffect, file.path(rootdir, "datasets/trueEff/MNAR2", propName, effname), row.names = F)
    }
}