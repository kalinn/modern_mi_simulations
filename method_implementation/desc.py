...
class SAE(object):
    ...
    def __init__(self, dims, act='relu', 
        ...
        self.autoencoders,self.encoder,self.decoder=self.make_autoencoders()
    ...
    def make_autoencoders(self):
        ...
        return Model(inputs=x,outputs=y,name="AE"),Model(inputs=x,outputs=h,name="encoder"),Model(inputs=h,outputs=y,name="decoder")
    ...
    def extract_preds(self, x):
        """
        Extract features from the middle layer of autoencoders(representation) and then decode.
        
        Arguments:
        x: data
        """
        en = self.encoder.predict(x)
        return self.decoder.predict(en)
