...
# Tuning params
theta = 7 # How many nodes to add at each layer of encoder
num_epochs = 500
dropout_ratio = 0.5
...
for x in range(1, 11):
    ...
    if simple=='TRUE':
        filename = "".join((fp, "final_simple/", "AE", mech, "/", str(propName), "/", "result", str(j), "_num_", str(x), ".csv"))
    else:
        filename = "".join((fp, "final_results/", "AE", mech, "/", str(propName), "/", "result", str(j), "_num_", str(x), ".csv"))
